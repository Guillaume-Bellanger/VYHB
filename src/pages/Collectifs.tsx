...
function CollectifSkeletons() {
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-5">
      {Array.from({ length: 6 }).map((_, i) => (
        <Skeleton key={i} className="h-[21rem] rounded-2xl bg-white/[0.04]" />
      ))}
    </div>
  );
}
            ...
            {collectifs.map((c, i) => {
              ...
              return (
                ...
                >
                  ...
                  >
                    {/* Gradient header */}
                    <div className={`relative h-48 bg-gradient-to-br ${getCollectifGradient(c.slug)} flex items-center justify-center overflow-hidden`}>
                      {c.photo_url && (
                        <img
                          src={c.photo_url}
                          alt={c.nom}
                    ...
                    <div className="p-5">
                      ...
                      <p className="text-xs text-orange-400 font-display font-semibold uppercase tracking-wider mb-3">{c.sous_titre}</p>
                      <p className="text-sm text-white/40 line-clamp-2 leading-relaxed mb-4">{c.description}</p>
