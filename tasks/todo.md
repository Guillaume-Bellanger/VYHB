...
- [ ] **ACTION UTILISATEUR EN ATTENTE** : exécuter le diagnostic `unnest(categories)` sur `encadrement` (déjà dans `018_categories.sql`, en bas du fichier) et communiquer le résultat pour compléter l'étape 3 (`UPDATE encadrement.categories[]`). Confirmer aussi si `collectifs.nom` doit être corrigé pour le même typo d'accent (question distincte de la décision "pas de restructuration de collectifs" — ceci n'est qu'une correction orthographique, pas une reclassification)
